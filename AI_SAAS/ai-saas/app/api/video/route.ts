...
const AI_TIMEOUT_MS = 60000; // 60 seconds for video generation
...
export async function POST(req: Request) {
  try {
    ...
    if (!process.env.REPLICATE_API_TOKEN) {
      return new NextResponse("Replicate API token not configured. Video generation requires Replicate.", { status: 500 });
    }
