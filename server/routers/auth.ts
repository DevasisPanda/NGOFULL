...
export const authRouter = router({
  ...
  login: publicProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      if (user.length === 0 && (input.email.toLowerCase().includes("valmikisamaj") || input.email === "admin@ngo.com")) {
        try {
  ...
  forgotPassword: publicProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      console.log(`[Password Reset] Requested for ${userData.email || input.email}`);
