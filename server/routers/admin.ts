...
export const adminRouter = router({
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      if (input.phone) {
        const welcomeMessage = `Hello ${input.name},\n\n` +
          `You have been successfully registered as a member of *Valmiki Samaj Charitable Trust*.\n\n` +
        ...
        sendWhatsAppMessage(input.phone, "Welcome to Valmiki Samaj Charitable Trust", welcomeMessage)
          .catch(err => {
  ...
  blockUser: adminProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      if (targetUser[0].role === "admin" && targetUser[0].id <= ctx.user.id && !ctx.user.isSystemAdmin) {
